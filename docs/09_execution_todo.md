...
- 状态：`[~]` 结构化日志 + 指标 + 告警（延迟/错误率/成本/parity rate）  
  - 已完成：HTTP 请求结构化日志（JSON）与内存指标采集，新增 `/metrics` 指标接口。
  - 未完成：外部指标后端接入、阈值告警与成本/parity rate 指标上报。
- 状态：`[~]` artifact 生命周期管理（清理策略/归档策略）
  - 已完成：`scripts/cleanup_artifacts.py` 与清理策略实现（按保留天数 + 每 agent 最新 N 个保留，支持 dry-run）。
  - 未完成：归档分层（冷热分离）与自动调度策略。
