...
- 状态：`[~]` 跑通真实 runtime + 真实 judge 的 parity 主实验  
  ...
  - 已完成：新增 `scripts/run_research_pipeline.py`，可一键编排矩阵->对照->parity->failure->recompute->gate 并生成 pipeline 报告。  
...
- 状态：`[~]` 建立 failure taxonomy（不是只看均分）  
  ...
  - 已完成：failure taxonomy 规则支持版本化 JSON（`docs/benchmarks/failure_taxonomy_rules_v1.json`），并可通过 `scripts/recompute_failure_taxonomy.py` 离线重算对比。  
...
- 状态：`[~]` 形成 research idea backlog（至少 3 个可证伪假设）并做小步 ablation  
  ...
  - 已完成：新增 `scripts/evaluate_hypothesis.py` 与 `docs/benchmarks/hypothesis_idea1_v1.json`，可自动输出 hypothesis 支持/证伪判定。  
